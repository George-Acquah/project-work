...
interface _IParkingCenterDetails {
  role: UserType;
}
const ParkingCenterDetailsComponent = ({ role }: _IParkingCenterDetails) => {
    ...
    <Button
      additionalStyles={
        role === UserType.PARK_OWNER
          ...
          : [styles.userOptionsButton, { borderRadius: SIZES.radius * 1.8 }]
      }
      additionalTextStyles={{
        color: SHARED_COLORS.gray50,
      }}
      type="opacity"
      title="Request for a Slot"
      onPress={() => {
    ...
    <RendererHOC loading={loading} error={error}>
      <>
        <ParallaxScrollView
          ...
          headerContent={
            <CenterDetailsHeaderImage center_images={[center?.image ?? ""]} />
          }
...
export default ParkingCenterDetailsComponent;
